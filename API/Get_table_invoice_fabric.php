...
$sqlFid = mysqli_query($con,"SELECT id FROM tbl_exim WHERE listno = '".$requestData['listno']."' LIMIT 1");
$dataId = mysqli_fetch_array($sqlFid);
...
while ($row = mysqli_fetch_array($query)) {
    $nestedData = array();
    $sql_KYP = mysqli_query($con,"SELECT sum(if(a.sisa='FOC',0,a.weight)) as kgs , sum(if(a.sisa='FOC',0,a.yard_)) as yds,sum(if(a.sisa='FOC',0,b.netto)) as pcs,
    ...
    $row_kyp = mysqli_fetch_array($sql_KYP);
    if ($row['price_by'] == "KGS") {
