<?php
$listno = strtoupper(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $_GET['listno'])));
header("Content-type: application/octet-stream");
header("Content-Disposition: attachment; filename=CI_NO_" . $listno . ".xls"); //ganti nama sesuai keperluan
...
<head>
    ...
    <title>PRINT COMMERCIAL INVOICE <?php echo $_GET['listno'] ?></title>
...
<body>
    ...
    <?php
    $sql = sqlsrv_query($con, "SELECT * FROM db_qc.tbl_exim WHERE listno='$_GET[listno]'");
    $r = sqlsrv_fetch_array($sql);
    $sql3 = sqlsrv_query($con, "SELECT * FROM db_qc.tbl_exim_si WHERE no_si='$r[no_si]'");
    ...
    <table width="100%" border="1" class="table-list1" style="width:7.8in">
        ...
        <?PHP $sql1 = sqlsrv_query($con, " SELECT DISTINCT 
            b.no_po, 
            b.no_order, 
            b.no_item,
            b.deskripsi
        FROM db_qc.tbl_exim a
