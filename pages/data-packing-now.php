		...
		<div class="box-body">
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<select name="byer" id="byer" class="form-control" onchange="window.location='index1.php?p=Data-Packing-NOW&amp;bon=<?php echo $_GET['bon']; ?>&amp;byer='+this.value">
							...
							?>
								<option value="<?php echo trim($rowdb2['ORDERPARTNERBRANDCODE']); ?>" <?php if (trim($rowdb2['ORDERPARTNERBRANDCODE']) == $_GET['byer']) {
																											echo "SELECTED";
																										} ?>><?php echo $rowdb2['ORDERPARTNERBRANDCODE']; ?></option>
							<?php } ?>
				...
				GROUP BY sl.EXTERNALREFERENCE,s.EXTERNALREFERENCE";
							...
							?>
								<option value="<?php echo urlencode($POLG); ?>" <?php if (urldecode($_GET['nopo']) == $POLG) {
																					echo "SELECTED";
																				} ?>><?php echo $POLG; ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
...
GROUP BY sl.SUBCODE02,sl.SUBCODE03,o.LONGDESCRIPTION";
							...
							while ($row2db2 = db2_fetch_assoc($stmt2)) { ?>
								<option value="<?php echo $row2db2['ITM']; ?>" <?php if ($row2db2['ITM'] == $_GET['itm']) {
																					echo "SELECTED";
																				} ?>><?php echo $row2db2['ITM'] . " | " . $row2db2['ITMADS']; ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
...
AND (sl.EXTERNALREFERENCE ='$_GET[nopo]' OR s.EXTERNALREFERENCE ='$_GET[nopo]' ) AND CONCAT(trim(sl.SUBCODE02),trim(sl.SUBCODE03))='$_GET[itm]'";
							...
							while ($row3db2 = db2_fetch_assoc($stmt3)) { ?>
								<option value="<?php echo $row3db2['ORDERLINE']; ?>" <?php if ($row3db2['ORDERLINE'] == $_GET['wrn']) {
																						echo "selected";
																					} ?>><?php echo $row3db2['LONGDESCRIPTION']; ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
					...
					<div class="col-sm-3">
						<select name="lot" id="lot" class="form-control" onchange="window.location='index1.php?p=Data-Packing-NOW&amp;bon=<?php echo $_GET['bon']; ?>&amp;byer=<?php echo $_GET['byer']; ?>&amp;nopo=<?php echo urlencode($_GET['nopo']); ?>&amp;itm=<?php echo $_GET['itm']; ?>&amp;wrn=<?php echo $_GET['wrn']; ?>&amp;lot='+this.value">
							...
							while ($row4db2 = db2_fetch_assoc($stmt4)) { ?>
								<option value="<?php echo $row4db2['CODE']; ?>" <?php if ($row4db2['CODE'] == $_GET['lot']) {
																					echo "selected";
																				} ?>><?php echo $row4db2['CODE']; ?></option>
							<?php } ?>
						</select>
					</div>
					<div class="col-sm-6">
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<select name="lot" id="lot" class="form-control" onchange="window.location='index1.php?p=Data-Packing-NOW&amp;bon=<?php echo $_GET['bon']; ?>&amp;byer=<?php echo $_GET['byer']; ?>&amp;nopo=<?php echo urlencode($_GET['nopo']); ?>&amp;itm=<?php echo $_GET['itm']; ?>&amp;wrn=<?php echo $_GET['wrn']; ?>&amp;lot=<?php echo $_GET['lot']; ?>&amp;lot1='+this.value">
							...
							$stmt4 = db2_exec($conn1, $sql4DB2, array('cursor' => DB2_SCROLLABLE));
							while ($row4db2 = db2_fetch_assoc($stmt4)) { ?>
								<option value="<?php echo $row4db2['CODE']; ?>" <?php if ($row4db2['CODE'] == $_GET['lot1']) {
																					echo "selected";
																				} ?>><?php echo $row4db2['CODE']; ?></option>
							<?php } ?>
						</select>
			...
			ORDER BY ITXVIEWSTOCKBALANCEKFF.CREATIONDATETIME DESC";
								...
								?>
									...
									<tr>
										...
										<!--<td align="center" ><?PHP if ($_GET['pk'] == "Rolls" and substr($rowd['ELEMENTSCODE'], 0, 1) != "2") {
																	echo substr($rowd['ELEMENTSCODE'], 8, 5);
																} else {
																	echo "-";
																} ?></td>
      <td align="center" ><?PHP if ($_GET['pk'] == "Bales" and substr($rowd['ELEMENTSCODE'], 0, 1) != "2") {
										echo substr($rowd['ELEMENTSCODE'], 8, 5);
									} else {
										echo "-";
									} ?></td>-->
										...
										<td align="right"><?PHP if ($rowd['BASEPRIMARYQUANTITYUNIT'] == "") {
																echo "-";
															} else {
																echo number_format(round($rowd['BASEPRIMARYQUANTITYUNIT'], 2) + $gw, '2', '.', ',');
															} ?></td>
										<td align="right"><?PHP
															if (trim($rw2d['LONGDESCRIPTION']) != '') {
																if (trim($rw2d['LONGDESCRIPTION']) == "FOC") {
																	...
																	trim($rw2d['LONGDESCRIPTION']) == "Sisa Ganti Kain" or trim($rw2d['LONGDESCRIPTION']) == "Sisa MOQ"
