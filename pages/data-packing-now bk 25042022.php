...
<div class="box box-info">	
     ...
     ?>
	  ...
	  <?php } ?>
	   ...
	   ?>
		<option value="<?php echo urlencode($row1db2['PO2']);?>" <?php if(urldecode($_GET['nopo'])==$row1db2['PO2']){ echo "SELECTED";} ?>><?php echo $row1db2['PO2'];?></option>				
	  <?php } ?>
					</select>
				</div>
			</div>	
...
onchange="window.location='index1.php?p=Data-Packing-NOW&amp;bon=<?php echo $_GET['bon'];?>&amp;byer=<?php echo $_GET['byer']; ?>&amp;nopo=<?php echo urlencode($_GET['nopo']); ?>&amp;itm='+this.value">
		...
		$sql2DB2="SELECT CONCAT(trim(sl.SUBCODE02),trim(sl.SUBCODE03)) AS ITM FROM SALESORDER s
LEFT JOIN SALESORDERLINE sl ON s.CODE=sl.SALESORDERCODE 
WHERE s.CODE ='$_GET[bon]' AND s.ORDERPARTNERBRANDCODE='$_GET[byer]' 
AND (sl.EXTERNALREFERENCE ='$npo' OR s.EXTERNALREFERENCE ='$npo' )
GROUP BY sl.SUBCODE02,sl.SUBCODE03";
		$stmt2=db2_exec($conn1,$sql2DB2, array('cursor'=>DB2_SCROLLABLE));
		while($row2db2 = db2_fetch_assoc($stmt2)) { ?>			  
				    <option value="<?php echo $row2db2['ITM'];?>" <?php if($row2db2['ITM']==$_GET['itm']){ echo "SELECTED"; }?>><?php echo $row2db2['ITM'];?></option>
		<?php } ?>	    
			      </select>
				</div>
			</div>
...
AND (sl.EXTERNALREFERENCE ='$_GET[nopo]' OR s.EXTERNALREFERENCE ='$_GET[nopo]' ) AND CONCAT(trim(sl.SUBCODE02),trim(sl.SUBCODE03))='$_GET[itm]'";
		...
		while($row4db2 = db2_fetch_assoc($stmt4)) { ?>
			...
			<div class="form-group">
				...
				<div class="col-sm-2">
				  <select name="packing" id="packing" class="form-control" onchange="window.location='index1.php?p=Data-Packing-NOW&amp;bon=<?php echo $_GET['bon'];?>&amp;byer=<?php echo $_GET['byer'];?>&amp;nopo=<?php echo urlencode($_GET['nopo']);?>&amp;itm=<?php echo $_GET['itm'];?>&amp;wrn=<?php echo $_GET['wrn'];?>&amp;lot=<?php echo $_GET['lot'];?>&amp;pk='+this.value">
				    <option value="">PILIH</option>
			...
			<div class="form-group">
				...
				<div class="col-sm-4">
				<?php 
		$sql5DB2="SELECT CONCAT(trim(WHSLOCATIONWAREHOUSEZONECODE), trim(WAREHOUSELOCATIONCODE)) AS TEMPAT FROM STOCKTRANSACTION p WHERE p.ITEMTYPECODE ='KFF' AND p.PROJECTCODE='$_GET[bon]' AND p.ORDERCODE='$_GET[lot]'";
...
<?php
$sqlT1DB2="SELECT COUNT(p.ITEMELEMENTCODE) AS ROLL, ROUND(SUM(p.BASEPRIMARYQUANTITY),2) AS BERAT,ROUND(SUM(p.BASESECONDARYQUANTITY),2) AS YARD FROM STOCKTRANSACTION p WHERE p.PROJECTCODE='$_GET[bon]' AND p.ORDERCODE='$_GET[lot]' AND p.ITEMTYPECODE ='KFF'
";
...
<table id="example5" class="table table-bordered table-hover table-striped" width="100%">
   ...
   <?php
	...
	$n=1;
	$c=0;
	$sqlC1DB2="SELECT * FROM STOCKTRANSACTION p WHERE p.PROJECTCODE='$_GET[bon]' AND p.ORDERCODE='$_GET[lot]' AND ITEMTYPECODE ='KFF'";
    ...
    <tr>
      ...
      <td align="center" ><?PHP if($_GET['pk']=="Rolls"){echo substr($rowd['ITEMELEMENTCODE'],8,3);}else{echo"-";} ?></td>
      <td align="center" ><?PHP if($_GET['pk']=="Bales"){echo substr($rowd['ITEMELEMENTCODE'],8,3);}else{echo"-";} ?></td>
      <td align="right" ><?PHP echo number_format(round($rowd['BASEPRIMARYQUANTITY'],2),'2','.',','); ?></td>
      <td align="right" ><?PHP echo number_format(round($rowd['BASESECONDARYQUANTITY'],2),'2','.',','); ?></td>
      ...
      <td align="right" ><?PHP if($rowd['BASEPRIMARYQUANTITY']==""){echo"-";}else{echo number_format(round($rowd['BASEPRIMARYQUANTITY'],2)+$gw,'2','.',',');} ?></td>
      <td align="right" ><?PHP 
	  if($rowd['sisa']=="FOC"){
...
<table id="example5" class="table table-bordered table-hover table-striped" width="100%">
   ...
   <?php    
    ...
    <tr>
      ...
      <td align="right" ><?PHP echo number_format(round($rowd['BASESECONDARYQUANTITY'],2),'2','.',','); ?></td>
      <td align="right" ><?PHP if($rowd['BASEPRIMARYQUANTITY']==""){echo"-";}else{echo number_format(round($rowd['BASEPRIMARYQUANTITY'],2)+$gw,'2','.',',');} ?></td>
      <td align="right" ><?PHP echo number_format(round($rowd['BASESECONDARYQUANTITY'],2)*0.9144,'2','.',','); ?></td>
    </tr>
