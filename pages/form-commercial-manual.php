...
if (isset($_POST['save'])) {
	...
	$nobl = $_POST['no_bl'];
	if ($_POST['tgl_si'] != "") {
		$tglsi = " tgl_si='" . $_POST['tgl_si'] . "',";
	} else {
		$tglsi = "tgl_si=null,";
	}
	if ($_POST['tgl_sj'] != "") {
		$tglsj = " tgl_sj='" . $_POST['tgl_sj'] . "',";
	} else {
		$tglsj = "tgl_sj=null,";
	}
	if ($_POST['tgl_peb'] != "") {
		$tglpeb = " tgl_peb='" . $_POST['tgl_peb'] . "',";
	} else {
		$tglpeb = "tgl_peb=null,";
	}
	if ($_POST['tgl_etd'] != "") {
		$tgletd = " etd='" . $_POST['tgl_etd'] . "',";
	} else {
		$tgletd = "etd=null,";
	}
	...
	// 	author='" . $_POST['author'] . "',
	// 	tgl_update=GETDATE()
	// 	"); //########### siapa yang bikin query insert seperti ini ???


	$qry1 = sqlsrv_query($con, "INSERT INTO db_qc.tbl_exim_cim (
				no_invoice, 
				consignee, 
				fasilitas, 
				no_si, 
  ...
  if (result.value) {
    window.location.href='?p=Commercial-Invoice-Manual';
		...
		echo "There's been a problem: " . sqlsrv_errors();
	}
}
if (isset($_POST['edit'])) {
	$note = $_POST['note'];
		...
		<div class="box-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<select name="consignee" class="form-control" required>
							...
							?>
								<option value="<?php echo strtoupper($rcon['nama']); ?>" <?php if ($dCI['consignee'] == strtoupper($rcon['nama'])) {
																								echo "SELECTED";
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
						<select name="fasilitas" class="form-control" required>
							<option value="">Pilih</option>
							<option value="UMUM" <?php if ($dCI['fasilitas'] == "UMUM") {
														echo "SELECTED";
													} ?>>UMUM</option>
							<option value="KITE" <?php if ($dCI['fasilitas'] == "KITE") {
														echo "SELECTED";
													} ?>>KITE</option>
							<option value="UMUM(KITE)" <?php if ($dCI['fasilitas'] == "UMUM(KITE)") {
															echo "SELECTED";
														} ?>>UMUM(KITE)</option>
						</select>
					</div>
			...
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						<input name="author" type="text" class="form-control" id="author" value="<?php if ($dCI['author'] == "") {
