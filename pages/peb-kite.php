          ...
          <table id="example2" class="table table-bordered table-hover table-striped nowrap" style="width:100%">
            <thead class="bg-red">
              <tr>
                ...
                <th width="87">
                  <div align="center">Tgl LHPRE</div>
                </th>
                <th width="72">
                  <div align="center">Tgl Expiry</div>
                </th>
                <th width="130">
                  <div align="center">BCL.KT02</div>
                </th>
                <th width="130">
                  <div align="center">Tgl Pengajuan</div>
                </th>
                <th width="130">
            ...
            <tbody>
              ...
              $sql = sqlsrv_query($con, "SELECT 
                  *, 
                  DATEADD(DAY, 180, tgl_lhpre) AS tgl_expiry,
                  DATEDIFF(DAY, GETDATE(), DATEADD(DAY, 180, tgl_lhpre)) AS hari 
              FROM db_qc.tbl_exim_cim 
              WHERE fasilitas = 'KITE' OR fasilitas = 'UMUM(KITE)' 
              ORDER BY fasilitas, etd ASC");
              while ($r = sqlsrv_fetch_array($sql)) {
                ...
                if ($r['hari'] > 0) {
                  $jmlhari = $r['hari'] . " Hari lagi";
                } else {
                  $jmlhari = "sudah EXPIRED";
                }
                if ($r['hari'] >= 30) {
                  $warna = " label-success";
                } else if ($r['hari'] >= 0) {
                ...
                $sqlTA = sqlsrv_query($con, "SELECT sum(nilai) as tajukan FROM db_qc.tbl_exim_pengembalian a
	  INNER JOIN db_qc.tbl_exim_cim_detail b ON a.id_cimd=b.id
	  INNER JOIN db_qc.tbl_exim_cim c ON b.id_cim=c.id
	  WHERE c.no_invoice='" . $r['no_invoice'] . "' AND a.sts='Tidak diajukan'");
                $rTA = sqlsrv_fetch_array($sqlTA);
                $sqlAJ = sqlsrv_query($con, "SELECT sum(nilai) as tajukan FROM db_qc.tbl_exim_pengembalian a
	  INNER JOIN db_qc.tbl_exim_cim_detail b ON a.id_cimd=b.id
	  INNER JOIN db_qc.tbl_exim_cim c ON b.id_cim=c.id
	  WHERE c.no_invoice='" . $r['no_invoice'] . "' AND a.sts='Ajukan'");
                $rAJ = sqlsrv_fetch_array($sqlAJ);
                $sqlTglA = sqlsrv_query($con, "SELECT * FROM db_qc.tbl_exim_bclkt WHERE no_bclkt='" . $rCIMD['no_bclkt'] . "'");
              ...
              ?>
                <tr bgcolor="<?php echo $bgcolor; ?>">
                  ...
                  <td align="center"><?php echo $r['tgl_peb']; ?><br>
                    <em><?php echo $r['no_peb']; ?></em>
                  </td>
                  <td align="center"><?php echo $r['tgl_lhpre']; ?></br>
                    <em><?php echo $r['no_lhpre']; ?></em>
                  </td>
                  <td align="center"><?php echo $r['tgl_expiry']; ?><br><?php if ($rTglA['pengajuan'] != "") {
                                                                        } else {
                                                                          if ($r['tgl_expiry'] != "") { ?><span class="label <?php echo $warna; ?>"><?php echo $jmlhari; ?></span><?php }
                                                                                                                                                                                                                  } ?></td>
