    ...
    $r11=mysqli_fetch_array($sql11);
     ...
     ?>
		...
		$order=str_replace("'","''",$_GET['order']);
					if($order!=""){ $where=" `tbl_kite`.`no_order` = '".$_GET['order']."' ";}
					else{ $where="`tbl_kite`.`id`='' ";}
		$sqlkk=mysqli_query($con,"SELECT trim(nokk) as nokk
FROM `tbl_kite`
WHERE $where GROUP BY nokk");
while($rkk=mysqli_fetch_array($sqlkk)){?>
      <option value="<?php echo $rkk['nokk'];?>" <?php if($rkk['nokk']==$_GET['nokk']){echo"selected";}?>><?php echo $rkk['nokk'];?></option>
      <?php  } ?>
...
<?php
$snkk1=mysqli_query($con,"SELECT count(*) as roll,sum(net_wight) as berat,sum(yard_) as yard FROM tbl_tembakqty a 
...
<table id="example5" class="table table-bordered table-hover table-striped" width="100%">
	...
	<tbody>
   <?php
      $datacek=mysqli_query($con,"SELECT * FROM tbl_tembakqty a 
INNER JOIN detail_tembakqty b ON a.id=b.id_kite WHERE  nokkkite='".$_GET['nokk']."' ORDER BY no_roll ASC");
    ...
    <tr>
      ...
      <td align="right" ><?PHP if($rowd['net_wight']==""){echo"-";}else{echo number_format($rowd['net_wight']+$gw,'2','.',',');} ?></td>
      <td align="right" ><?PHP 
	  if($rowd['sisa']=="FOC"){
