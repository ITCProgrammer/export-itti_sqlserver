        ...
        <div class="box table-responsive">
          ...
          <div class="box-body">
            <table id="example2" class="table table-bordered table-hover table-striped nowrap" style="width:100%">
              <thead class="bg-red">
                <tr>
                  ...
                  <th width="87" rowspan="2">
                    <div align="center">BCLKT No.</div>                  </th>
                  <th colspan="2"><div align="center">BCLKT</div></th>
                  <th colspan="3"><div align="center">SKEP</div></th>
                  <th width="87" rowspan="2"><div align="center">SPM Diterima</div>                  </th>
                  <th width="72" rowspan="2"> <div align="center">Uang Masuk</div>                  </th>
                  <th width="130" rowspan="2"> <div align="center">Note</div>                  </th>
                  <th width="80" rowspan="2"><div align="center">Action</div></th>
                </tr>
                <tr>
                  <th width="186">
              ...
              <tbody>
                <?php
  $sql=mysql_query("SELECT * FROM tbl_exim_bclkt ORDER BY pengajuan DESC");
  while ($r=mysql_fetch_array($sql)) {
      $no++;
      $bgcolor = ($col++ & 1) ? 'gainsboro' : 'antiquewhite'; 
	  $sqlck=mysql_query("SELECT *,now() as tglskrng FROM tbl_exim_bclkt ");
	  $ck=mysql_num_rows($sqlck);
	  $rck=mysql_fetch_array($sqlck);
	  $akhir = strtotime($rck[tglskrng]);
	  
	  $awal  = strtotime($r[pengajuan]);	  
	  $diff  = ($akhir - $awal);		
	  $tjam  =round($diff / (60 * 60));
	  $hari  =floor($tjam/24);
	  
	  $awal1  = strtotime($r[disetujui]);	  
	  $diff1  = ($akhir - $awal1);		
	  $tjam1 =round($diff1 / (60 * 60));
	  $hari1  =floor($tjam1/24);
	  
	  $awal2  = strtotime($r[pencairan]);	  
	  $diff2  = ($akhir - $awal2);		
	  $tjam2  =round($diff2 / (60 * 60));
	  $hari2  =floor($tjam2/24);
	  
	  $awal3  = strtotime($r[terima_spm]);	  
	  $diff3  = ($akhir - $awal3);		
	  $tjam3  =round($diff3 / (60 * 60));
	  $hari3  =floor($tjam3/24);
	  $sqlHitung= mysql_query("SELECT sum(nilai) as tot FROM tbl_exim_cim a
INNER JOIN tbl_exim_cim_detail b ON a.id=b.id_cim
LEFT JOIN tbl_exim_pengembalian c ON b.id=c.id_cimd
WHERE c.sts='Ajukan' and b.no_bclkt='$r[no_bclkt]'");
	  $rHtg=mysql_fetch_array($sqlHitung);
	 ...
	 ?>
                <tr bgcolor="<?php echo $bgcolor; ?>">
                  ...
                  <td align="center"><?php echo $r[pengajuan]; ?></td>
                  <td align="center"><?php if($r[disetujui]!=""){echo $r[disetujui];}else if($r[pengajuan]!=""){ echo '<span class="label label-danger">'.$hari.' Hari</span>'; } ?></td>
                  <td align="center"><?php if($r[pencairan]!=""){echo $r[pencairan];}else if($r[disetujui]!=""){ echo '<span class="label label-danger">'.$hari1.' Hari</span>'; } ?></td>
                  <td align="center"><?php if($r[terima_spm]!=""){echo $r[terima_spm];}else if($r[pencairan]!=""){ echo '<span class="label label-danger">'.$hari2.' Hari</span>'; } ?></td>
                  <td align="center"><?php if($r[uang_msk]!=""){echo $r[uang_msk];}else if($r[terima_spm]!=""){ echo '<span class="label label-danger">'.$hari3.' Hari</span>'; } ?></td>
                  <td><?php echo $r[note]; ?></td>
                  <td align="center">
					<div class="btn-group">  
                    <a href="#" class="btn btn-xs btn-danger <?php
                    if(($_SESSION['levelEX']=="Manager" or $_SESSION['levelEX']=="SPV")){ echo "enabled" ; }
                    else { echo "disabled" ; } ?>" onclick="confirm_delBCLKT('?p=bclkt_hapus&id=<?php echo $r[id] ?>');"><i class="fa fa-trash"></i> </a>
                    ...
                    if($_SESSION['levelEX']=="Manager" or $_SESSION['levelEX']=="SPV"){ echo "enabled" ; }
                    else { echo "disabled" ; } ?>" target="_blank"><i class="fa fa-search"></i> </a>	
					</div></td>
                </tr>
                <?php
  } ?>
