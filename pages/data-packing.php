...
while($rp=sqlsrv_fetch_array($sqllot)){?>
	...
	sum( b.weight ) AS kg,
IF
	( a.typestatus = '2', 'kain masuk', 'kain keluar' ) AS ket,
	DATE_FORMAT( a.tgl_update, '%d-%m-%Y' ) AS tgl 
FROM
	pergerakan_stok a
	INNER JOIN detail_pergerakan_stok b ON a.id = b.id_stok 
WHERE
	( a.typestatus = '2' OR a.typestatus = '3' ) 
	AND b.nokk = '".$_GET['lot']."' 
GROUP BY
	a.id");
...
ORDER BY
    ...
    <tr>
      ...
      <td align="center" ><?PHP if($_GET['pk']=="Rolls"){echo $rowd['rolno'];}else{echo"-";} ?></td>
      <td align="center" ><?PHP if($_GET['pk']=="Bales"){echo $rowd['rolno'];}else{echo"-";} ?></td>
      ...
      <td align="right" ><?PHP if($rowd['weight']==""){echo"-";}else{echo number_format($rowd['weight']+$gw,'2','.',',');} ?></td>
      <td align="right" ><?PHP 
	  if($rowd['sisa']=="FOC"){
